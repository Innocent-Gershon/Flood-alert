...
const Sidebar = () => {
  ...
  // --- ✅ FIX #1: The Home link now correctly points to "/home" ---
  const navigation = [
  ...
  const confirmLogout = () => {
    ...
    // --- ✅ FIX #2: Logout now redirects to the "/login" page ---
    navigate('/login');
  };
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex flex-col bg-blue-900 text-white h-screen w-64 fixed left-0 top-0 shadow-lg">
        {/* --- ✅ FIX #3: The logo is now a clickable link to "/home" --- */}
        <Link to="/home" className="p-4 border-b border-blue-800 block hover:bg-blue-800 transition-colors">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-40">
          ...
          <div className="fixed inset-y-0 left-0 w-64 bg-blue-900 text-white shadow-lg flex flex-col">
            <Link to="/home" onClick={toggleMobileMenu} className="p-4 border-b border-blue-800 block">
                <div className="flex items-center space-x-2">
                    <img src={logoImage} alt="Flood Alert Ghana Logo" className="h-8 w-8" />
                    <h1 className="text-xl font-bold">Flood Alert Ghana</h1>
                </div>
